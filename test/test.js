...
const rm = require("router-middleware").default;
...
test("post - router-middleware", (t) => {
  t.plan(3);
  const app = rm();
  app.post("/user/:userId/messages", app.jsonParser(), (req, res, next) => {
    t.deepEquals(req.query, { language: "en" }, "Querystring is received");
    t.equals(req.params.userId, "a1234", "path parameters is received");
...
test.skip("fail - force error on socket", (t) => {
  t.plan(1);
  const express = require("express");
  const app = express();
  app.get("/foo", (req) => {
